...
import workOrdersFakerFactory from "./workOrdersFakerFactory";
...
const WorkOrdersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = workOrdersFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("workOrders").create(elem));
...
export default connect(mapState, mapDispatch)(WorkOrdersFakerDialogComponent);
