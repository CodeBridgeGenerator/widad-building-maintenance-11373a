...
import seederData from "./workOrders.seeder.json";
...
const WorkOrdersSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("workOrders").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(WorkOrdersSeederDialogComponent);
