...
import seederData from "./maintenanceHistory.seeder.json";
...
const MaintenanceHistorySeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("maintenanceHistory").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(MaintenanceHistorySeederDialogComponent);
