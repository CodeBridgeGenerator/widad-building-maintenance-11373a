...
import maintenanceHistoryFakerFactory from "./maintenanceHistoryFakerFactory";
...
const MaintenanceHistoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = maintenanceHistoryFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("maintenanceHistory").create(elem));
...
export default connect(mapState, mapDispatch)(MaintenanceHistoryFakerDialogComponent);
