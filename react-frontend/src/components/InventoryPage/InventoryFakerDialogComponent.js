...
import inventoryFakerFactory from "./inventoryFakerFactory";
...
const InventoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = inventoryFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("inventory").create(elem));
...
export default connect(mapState, mapDispatch)(InventoryFakerDialogComponent);
