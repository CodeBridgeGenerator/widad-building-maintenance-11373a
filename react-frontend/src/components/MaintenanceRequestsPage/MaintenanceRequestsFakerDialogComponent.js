...
import maintenanceRequestsFakerFactory from "./maintenanceRequestsFakerFactory";
...
const MaintenanceRequestsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = maintenanceRequestsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("maintenanceRequests").create(elem));
...
export default connect(mapState, mapDispatch)(MaintenanceRequestsFakerDialogComponent);
