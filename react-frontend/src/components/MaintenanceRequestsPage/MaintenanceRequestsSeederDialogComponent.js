...
import seederData from "./maintenanceRequests.seeder.json";
...
const MaintenanceRequestsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("maintenanceRequests").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(MaintenanceRequestsSeederDialogComponent);
