...
import buildingsFakerFactory from "./buildingsFakerFactory";
...
const BuildingsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = buildingsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("buildings").create(elem));
...
export default connect(mapState, mapDispatch)(BuildingsFakerDialogComponent);
