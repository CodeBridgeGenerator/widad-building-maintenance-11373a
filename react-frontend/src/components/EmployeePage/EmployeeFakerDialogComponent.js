...
import employeeFakerFactory from "./employeeFakerFactory";
...
const EmployeeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employee").create(elem));
...
export default connect(mapState, mapDispatch)(EmployeeFakerDialogComponent);
